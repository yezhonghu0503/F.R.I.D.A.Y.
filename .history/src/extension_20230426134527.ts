...
const collectCode = () => {
	const contextMenu:any = vscode.window.createQuickPick();
	contextMenu.items = [
		{
			label: '初代机',
			command: 'yourExtension.runCommand'
		}
	];
	contextMenu.show();
	...
	axios.get('http://129.226.201.240:3000/chat', { params: { msg: `${word}` } }).then(res => {
		...
		panel = vscode.window.createWebviewPanel('webview', 'My Webview', vscode.ViewColumn.Two, {});
		// 设置 Webview HTML 内容 

...
const getWebviewContent = (markdownMsg: any): string => {
	const html = highlightAuto(markdownMsg).value;
	const htmlcheerio = load(html);
	// const htmll = document.createElement('div');
	// htmll.innerHTML = html;
	// console.log(htmll);
	return ` <!DOCTYPE html>
	<html>
	<head>
...
const collectCode = () => {
	...
	axios.get('http://129.226.201.240:3000/chat', { params: { msg: `${question}` } }).then(res => {
		...
		// 设置 Webview HTML 内容 
		const uri = vscode.Uri.file(`${__dirname}\\index.html`)
